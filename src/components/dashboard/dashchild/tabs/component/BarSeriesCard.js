...
class BarSeries extends Component {
  ...
  async componentDidMount() {
    const { products, factPurchased, factSales, type } = this.props;
    let pivotTime = [];
    if (products) {
      if (type === "purchasing") {
        let passedValue = factPurchased;
        ...
        let unique = [...new Set(counterData.map(item => item.name))].sort();
  ...
  async componentDidUpdate(prevProps) {
    ...
    if (type === "purchasing") {
      if (!_.isEqual(prevProps.products, products)) {
        ...
        pivotTime = [...new Set(pivotTime)].sort();
        this.setState({
          counterPurchased: unique,
          counterTime: {
            pivotTime
            // min: pivotTime[0],
            // max: pivotTime[pivotTime.length - 1]
          }
        });
      }
    }
  ...
  render() {
    const { counterPurchased, counterTime, clickedValue } = this.state;
        ...
        ? counterPurchased.map(item => {
            return item.disabled ? null : (
              <VerticalBarSeries
                ...
                color={item.color}
                data={item.data}
                onSeriesClick={() => this.setState({ clickedValue: item })}
              />
            );
          })
        : counterPurchased.map(item => {
            return item.disabled ? null : (
              <VerticalBarSeries
                ...
                color={item.color}
                data={item.data}
                onSeriesClick={() => console.log(counterPurchased)}
              />
            );
          });
    ...
    /*  const valuePeriod =
             ...
             */
    if (!counterPurchased && !products && !counterTime) {
      return (
        <div>
      ...
      // let minX =
      //   counterTime.min === undefined ? "999999" : counterTime.min.toString();
      // let maxX =
      //   counterTime.max === undefined ? "0" : counterTime.max.toString();
          ...
          <Row>
            ...
            <Col style={{ height: "300px" }}>
              ...
              >
                ...
                {links}
                {clickedValue ? (
                  <Hint value={clickedValue}>
                    <div style={tipStyle}>{clickedValue.title}</div>
                  </Hint>
                ) : null}
              </FlexibleXYPlot>
            </Col>
